...
public class CloudletSchedulerDynamicWorkload extends CloudletSchedulerTimeShared {
	...
	public double getTotalUtilizationOfCpu(double time) {
		...
		int cloudlets = 0; // RD edit
		for (ResCloudlet rcl : getCloudletExecList()) {
			totalUtilization += rcl.getCloudlet().getUtilizationOfCpu(time);
			cloudlets++;
//			System.out.println(totalUtilization+" / "+cloudlets);
		}
		//dibagi cloudlet untuk mencegah return 0. Nampaknya ketika return 0, dia akan diremove. So lebih baik
		//return NaN (0/0). Nanti akan dihandle.
		if (Double.isNaN(totalUtilization/cloudlets)) return 0.00000001; //contohnya ini. SAFE TO REMOVE
		return totalUtilization/cloudlets; 
