...
public class Helper {
	...
	public static void printResults(
		...
		System.out.println("print power per host");
		for (int i = 0; i < hosts.size(); i++) {
			System.out.println(i+", "+((PowerHost)hosts.get(i)).getPower()+" ");
		}
	...
	protected static Map<String, Double> getSlaMetrics(List<Vm> vms) {
		...
		for (Vm vm : vms) {
			...
			for (VmStateHistoryEntry entry : vm.getStateHistory()) {
				if (previousTime != -1) {
					...
					if (previousAllocated < previousRequested) {
						double amount = (previousRequested - previousAllocated) / previousRequested;
						slaViolation.add(amount);
		...
		System.out.println("max sla "+max*100);
		System.out.println("min sla "+min*100);
	...
	public static void writeMetricHistory(
			...
			String outputPath) {
		// for (Host host : hosts) {
		for (int j = 0; j < hosts.size(); j++) { //10 hosts?
